...
let administrador = document.getElementById('permisoDeAdministracion');
...
passport.use('local.signup', new LocalStrategy({
    ...
    const { permisoDeAdministracion } = req.body;
    let newUser = {
        fullname,
        username, 
        password,
    ...
    pool.query('INSERT INTO users SET ? ', [newUser], function(error, results){
      ...
      return done(null, newUser, req.flash('success', 'Usuario creado correctamente!'));
    });
}));
...
formulario_objeto_st.addEventListener('submit', (e) =>{
    
  e.preventDefault();

  let admin = document.getElementById('permisoDeAdministracion').checked;
