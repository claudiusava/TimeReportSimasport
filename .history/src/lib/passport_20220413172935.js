...
passport.use('local.signup', new LocalStrategy({
  ...
  const consulta = await pool.query('SELECT id FROM users WHERE fullname = ?, username = ?, password = ? ', newUser.fullname, newUser.username, newUser.password);

  console.log(consulta);

  return done(null, consulta);
}));
